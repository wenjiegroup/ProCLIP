...
class Alphabet(object):
    ...
    def get_unmasked_converter(self):
        return UnmaskedConverter(self)
...
class UnmaskedConverter(object):
    """Callable to convert an unprocessed (labels + strings) batch to a
    processed (labels + tensor) batch.
    """
    ...
    def __call__(self, raw_batch: Sequence[Tuple[str, str]]):
        ...
        labels = np.array([[0 for i in range(max_len + 2)]])
        strs = np.array([[0 for i in range(max_len + 2)]])
